...
Router.post("/new", function (req, res, next) {
    ...
    semesterModel.createOne({ alias }, (err, semester) => {
        ...
        return jsonResponse({ req, res }).success({ statusCode: 200, message: "Semester with alias " + alias + " has been created successfully!", data: semester });
    });
});
...
Router.get("/get/:alias", function (req, res, next) {
    ...
    semesterModel.findOneByAlias({ alias }, (err, semester) => {
        if (err) return next(err);
        return jsonResponse({ req, res }).success({ statusCode: 200, message: "Get record of semester with alias " + alias + ".", data: semester });
    });
});
...
Router.delete("/delete/:alias", function (req, res, next) {
    ...
    semesterModel.deleteOneByAlias({ alias }, (err, semester) => {
        if (err) return next(err);
        return jsonResponse({ req, res }).success({ statusCode: 200, message: "Semester with alias " + alias + " has been removed." });
    });
});
Router.delete("/delete/:alias/force", function (req, res, next) {
    ...
    semesterModel.forceDeleteOneByAlias({ alias }, (err, semester) => {
        if (err) return next(err);
        return jsonResponse({ req, res }).success({ statusCode: 200, message: "Semester with alias " + alias + " has been removed out of database." });
    });
});
...
Router.put("/restore/:alias", function (req, res, next) {
    ...
    semesterModel.restoreOneByAlias({ alias }, (err, course) => {
        if (err) return next(err);
        return jsonResponse({ req, res }).success({ statusCode: 200, message: "Semester with alias " + alias + " has been restored." });
    });
});
