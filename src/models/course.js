...
class Course {
    ...
    deleteOneByCode({ code = null }, callback) {
        ...
        this.model.findOne({ code }, (error, course) => {
            ...
            if (!course) return callback(throwError({ name: "NotFound", message: "Course code " + code + " cannot be found.", status: 404 }), null);
            return this.model.delete({ code }, (error, course) => {
                if (error) return callback(throwError({ error }), null);
                return callback(null, course);
            });
        });
    }
    restoreOne({ code = null }, callback) {
        if (!code) return callback(throwError({ name: "MissedContent", message: "Course code must be provided.", status: 200 }), null);
        this.model.findDeleted({ code }, (error, course) => {
            if (error) return callback(throwError({ error }), null);
            if (!course[0]) return callback(throwError({ name: "NotFound", message: "Course code " + code + " cannot be found.", status: 404 }), null);
    ...
    findOneByCode({ code = null }, callback) {
        ...
        this.model.findOne({ code }, (error, course) => {
            if (error) return callback(throwError({ error }), null);
            if (course) return callback(null, course);
            return callback(throwError({ name: "NotFound", message: "Course code " + code + " cannot be found.", status: 404 }), null);
        });
    }
    createOne({ code = null, name = null, credits = null, description = null, prerequisite = [], timeAllocation = {} }, callback) {
        ...
        this.findOneByCode({ code }, async (err, course) => {
            if (course) return callback(throwError({ name: "DatabaseError", message: `${course.name} is already existed.`, status: 200 }), null);
            const newCourse = await this.model.create({
                ...
                description,
                prerequisite,
                timeAllocation: { theory: timeAllocation?.theory || 1, practice: timeAllocation?.practice || 1, selfStudy: timeAllocation?.selfStudy || 1 },
            });
